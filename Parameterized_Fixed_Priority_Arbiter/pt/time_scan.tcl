...
##################################################################
# Report Timing
redirect -tee -file [format "%s%s"  [format "%s%s"  [format "%s%s"  $report_dir $design_name] $tech_lib] "${my_type}_pt_rt.rpt"] { report_timing -sign 4 -pba_mode exhaustive -path_type full -delay_type max -sort_by slack }
redirect -tee -append -file [format "%s%s"  [format "%s%s"  [format "%s%s"  $report_dir $design_name] $tech_lib] "${my_type}_pt_rt.rpt"] { report_timing -sign 4 -pba_mode exhaustive -path_type full -delay_type max -sort_by slack -from [all_registers -clock_pins] -to [all_registers -data_pins] }
